...
- Fehlende Pflicht-Nodes (`LoadImage`, `RepeatImageBatch`, `SaveImage`, `VIDAX_Wav2Lip`, optional `VHS_LoadVideo` bei Video-Start) oder fehlende Wav2Lip-Weights lösen `COMFYUI_BAD_RESPONSE` aus; bei aktivem ComfyUI gibt es keinen Fallback-Encode. Wenn `VIDAX_Wav2Lip` laut `object_info` fehlt, signalisiert der Doctor zusätzlich einen ComfyUI-Restart-Bedarf.
