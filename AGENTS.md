...
- Runner enforces resume/overwrite rules, measures audio via `ffprobe`, und performt echtes ffmpeg-Muxing (CFR `determinism.fps`, Audio-Padding für Buffer, Frame-Hold für Post). Visual-Priorität: `start_video` als Primärquelle (kein Standbild-Extract), danach ComfyUI-Video/Frames; fällt ComfyUI aus und nur `start_image` existiert, erzeugt der Runner ein Seed-deterministisches Ken-Burns-Motion-Fallback, der Still-Dummy bleibt letzter Notanker. Scaling richtet sich nach der abgeleiteten Render-Auflösung; Endbild-Hold (`end_image` + Post-Puffer) wird per re-encoded Concat (scale/fps/yuv420p) stabil an das Hauptvideo gehängt.
