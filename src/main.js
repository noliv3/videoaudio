...
yargs(hideBin(process.argv))
  .command('validate <job>', 'validate job.json', (y) => y.positional('job', { type: 'string' }), (args) => {
    ...
    result.errors.forEach((err) => {
      console.error(`${err.code || result.code}: ${err.field} -> ${err.message}`);
    });
    process.exit(10);
  })
  .command('run <job>', 'run job locally', (y) => y.positional('job', { type: 'string' }).option('workdir', { type: 'string' }), async (args) => {
    try {
      ...
      if (err.details?.errors) {
        err.details.errors.forEach((e) => console.error(`${e.code}: ${e.field} -> ${e.message}`));
      }
      process.exit(1);
    }
  })
