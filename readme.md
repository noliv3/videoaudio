...
- Encode: Reales ffmpeg-Muxing (CFR auf `determinism.fps`) mit gepaddeter Audio-Timeline, Scaling auf die abgeleitete Render-Auflösung, optional Endbild-Hold im Post-Puffer (wenn `end_image` und `post_seconds>0`), Post-Puffer via Frame-Hold, Dummy-Video aus Startbild/-frame falls keine ComfyUI-Frames/Videos.
