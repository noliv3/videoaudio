...
2. `node src/main.js install` legt StateDir + Configs an, zieht Assets gemäß Assets-Manifest (Auflösung: `VIDAX_ASSETS_CONFIG` → `VA_STATE_DIR/state/config/assets.json` → `config/assets.json`; StateDir via `VA_STATE_DIR`, default `~/.va/state`) und installiert optional ComfyUI-Custom-Nodes (`--no-comfy-nodes` oder `VA_INSTALL_COMFY_NODES=false` zum Abschalten). `COMFYUI_DIR` setzt das Ziel für Custom Nodes (default `F:\ComfyUI\custom_nodes` unter Windows, sonst `~/.va/state/comfyui/custom_nodes`).
3. `node src/main.js produce --audio <A> --start <img|vid> [--end <img>] [--pre/post ...] [--width/height ...] [--max_width/max_height ...] [--lipsync on|off --lipsync_provider <id>] [--comfyui_chunk_size N] [--no-comfyui] [--workdir ...]` baut ein Job-JSON und startet den Run (Final-Basename `fertig.mp4`). Default: ComfyUI an, Lipsync aus; `--no-comfyui` nur für Diagnosefälle. Prompt/Negative sind optional und werden vom Default-Workflow nicht benötigt.
...
- ComfyUI: Pflicht im Produktionspfad (Health-Check, Fehler → `COMFYUI_UNAVAILABLE`, kein Dummy-Skip). Inline API-Graphen bauen Audio-getriebene Wav2Lip-Workflows: Startbild → `RepeatImageBatch` (`frame_count=target_frames`) → `LoadAudio` → `Wav2Lip` → `VHS_VideoCombine(format=video/mp4, frame_rate=fps, ohne Audio)`, Startvideo → `VHS_LoadVideo(force_rate=fps, frame_load_cap=target_frames, force_size=Custom w/h)` → `LoadAudio` → `Wav2Lip` → `VHS_VideoCombine`. Outputs werden primär als `workdir/comfyui/comfyui_video.mp4` geladen; falls Frames geliefert werden, landen sie in `workdir/frames/` und werden strikt als `000001.png`, `000002.png`, ... nummeriert. Manifest zeichnet `chunk_size=target_frames`/`chunk_count=1`, `workflow_id`, `prompt_id` und Output-Pfade auf; Uploads nutzen `/upload/image` für Startbild/-video und Audio. Default-IDs: `vidax_wav2lip_image_audio` (Startbild) bzw. `vidax_wav2lip_video_audio` (Startvideo).
...
- Encode: Reales ffmpeg-Muxing (CFR auf `determinism.fps`) mit gepaddeter Audio-Timeline, Scaling auf die abgeleitete Render-Auflösung, optional Endbild-Hold im Post-Puffer (wenn `end_image` und `post_seconds>0`), Post-Puffer via Frame-Hold. Visual-Priorität: Bei aktivem ComfyUI wird das erzeugte `comfyui_video.mp4` (oder die daraus konvertierten Frames) encodiert; bei deaktiviertem ComfyUI bleibt `start_video` Primärquelle, Startbild löst einen Ken-Burns-Motion-Fallback aus, Still-Dummy bleibt letzter Notanker. End-Holds werden via re-encoded Concat (scale/fps/yuv420p) an die Hauptspur gehängt, damit Startvideo + Hold stabil bleiben.
