...
4. Alternativ: `node src/main.js validate path/to/job.json` und `node src/main.js run path/to/job.json --workdir /abs/workdir`
...
- CLI-Kommandos `produce`, `validate`, `run`, `status`, `logs` mit Exit-Codes aus `src/errors.js`; `produce` baut/validiert ein Job-JSON aus Flags und startet sofort (Workdir default `./workdir/run-<ts>`, Final `fertig.mp4`), `run` kennt `--workdir` und `--resume`.
...
- ComfyUI: Default-Workflow `vidax_text2img_frames` (wenn `workflow_ids` leer) erzeugt SDXL-Text2Img-Batches (`frame_count = target_frames`, Output PNG-Frames) und erwartet ein SDXL-Checkpoint (z. B. `sd_xl_base_1.0.safetensors`) im Models-Verzeichnis. Parameter-Mapping: Prompt aus `comfyui.params.prompt` oder `motion.prompt`, Negative aus `comfyui.params.negative(_prompt)`, Auflösung 1024x576 (override via `comfyui.params.width/height`), `steps` (Default 20), `cfg` (Default `motion.guidance` oder 7.5), `sampler` (Default `dpmpp_2m`), `scheduler` (Default `karras`), Seed laut Policy. `submit → wait (poll_interval_ms default 500) → collect` schreibt nach `workdir/comfyui/output.mp4` oder `workdir/frames/`; fehlende Assets blocken Auto-Start, nicht jedoch bereits laufende ComfyUI-Instanzen mit `ok` Health.
- LipSync: Läuft nur bei `lipsync.enable` (default true) **und** vorhandenem Provider in `config/lipsync.providers.json`; Output `workdir/lipsync/output.mp4`; `allow_passthrough=true` erlaubt Encode trotz Fehler; verarbeitet gepaddete Audioquelle.
